//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import "WACloudMediaDownloadRequest.h"

@class WAVideoConverter;

@interface WACloudVideoDownloadRequest : WACloudMediaDownloadRequest
{
    WAVideoConverter *_videoConverter;
}

@property(retain, nonatomic) WAVideoConverter *videoConverter; // @synthesize videoConverter=_videoConverter;
- (void).cxx_destruct;
- (void)cancel;
- (int)reallyRequestAssetFromPhotosLibraryWithCompletionHandler:(CDUnknownBlockType)arg1;

@end

