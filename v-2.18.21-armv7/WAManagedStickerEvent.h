//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <CoreData/NSManagedObject.h>

@class NSDate, NSString, WAManagedSticker;

@interface WAManagedStickerEvent : NSManagedObject
{
}

+ (id)fetchRequest;

// Remaining properties
@property(copy, nonatomic) NSString *fileHash; // @dynamic fileHash;
@property(retain, nonatomic) WAManagedSticker *sticker; // @dynamic sticker;
@property(copy, nonatomic) NSDate *timestamp; // @dynamic timestamp;
@property(nonatomic) long long type; // @dynamic type;

@end

