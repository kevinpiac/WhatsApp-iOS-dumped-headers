//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UINavigationController.h>

#import "SinglePersonPickerDelegate-Protocol.h"
#import "WAContactDetailsViewControllerDelegate-Protocol.h"
#import "WAContactEditViewControllerDelegate-Protocol.h"
#import "WAGroupComposerDelegate-Protocol.h"
#import "WAParticipantPickerViewControllerDelegate-Protocol.h"

@class NSString, WAGroupComposerViewController;
@protocol UINavigationControllerDelegate><WAChatComposerDelegate;

@interface WAChatComposer : UINavigationController <WAGroupComposerDelegate, SinglePersonPickerDelegate, WAContactEditViewControllerDelegate, WAParticipantPickerViewControllerDelegate, WAContactDetailsViewControllerDelegate>
{
    WAGroupComposerViewController *_groupComposer;
    unsigned int _originalMode;
    unsigned int _mode;
}

@property(nonatomic) unsigned int mode; // @synthesize mode=_mode;
- (void).cxx_destruct;
- (void)contactDetailsViewController:(id)arg1 startChatWithContact:(id)arg2;
- (void)contactDetailsViewController:(id)arg1 startCallWithJID:(id)arg2 withVideo:(BOOL)arg3;
- (void)doneAction:(id)arg1;
- (void)contactEditControllerDidSave:(id)arg1;
- (void)contactEditControllerDidCancel:(id)arg1;
- (void)addNewContact;
- (void)singlePersonPickerDidAskToCreateNewContact:(id)arg1;
- (void)singlePersonPickerDidAskToCreateNewGroup:(id)arg1;
- (void)singlePersonPickerDidAskToInviteFriends:(id)arg1;
- (void)singlePersonPickerDidCancel:(id)arg1;
- (void)singlePersonPicker:(id)arg1 didSelectContact:(id)arg2;
- (void)groupComposer:(id)arg1 didFinishWithSubject:(id)arg2 picture:(id)arg3;
- (void)participantPickerDidCancel:(id)arg1;
- (void)participantPicker:(id)arg1 didSelectContacts:(id)arg2;
- (id)groupParticipantsPicker;
- (id)popViewControllerAnimated:(BOOL)arg1;
- (id)initWithMode:(unsigned int)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(nonatomic) __weak id <UINavigationControllerDelegate><WAChatComposerDelegate> delegate; // @dynamic delegate;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned int hash;
@property(readonly) Class superclass;

@end

